...
function PasswordInput({ registration, error, placeholder }) {
  ...
  return (
    <div className="relative">
      <input type={show ? 'text' : 'password'} {...registration} className={`${inputCls} pr-9`} placeholder={placeholder} />
      <button type="button" onClick={() => setShow((s) => !s)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-sm max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
          <div>
            ...
            <PasswordInput registration={register('currentPassword', { required: 'Required' })} />
            {errors.currentPassword && <p className="text-xs text-red-500 mt-1">{errors.currentPassword.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">New Password</label>
            <PasswordInput registration={register('newPassword', { required: 'Required', minLength: { value: 4, message: 'Min 4 characters' } })} />
            {errors.newPassword && <p className="text-xs text-red-500 mt-1">{errors.newPassword.message}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Confirm New Password</label>
            <PasswordInput registration={register('confirmPassword', {
              required: 'Required',
