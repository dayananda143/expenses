...
import { Egg, Plus, Pencil, Trash2, X, Check, ChevronLeft, ChevronRight } from 'lucide-react';

const EGGS_API = '/api/poultry/eggs';
...
function EggModal({ initial, flocks, onClose, onSave }) {
  const [form, setForm] = useState(initial ?? { flock_id: '', date: today(), count: '', notes: '' });
  ...
  async function submit(e) {
    ...
    const url = initial ? `${EGGS_API}/${initial.id}` : EGGS_API;
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-sm shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          <h2 className="font-semibold text-gray-900 dark:text-white">{initial ? 'Edit Entry' : 'Log Eggs'}</h2>
        ...
        <form onSubmit={submit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="date" value={form.date} onChange={e => f('date', e.target.value)}
                className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Egg Count *</label>
              <input type="number" min="0" value={form.count} onChange={e => f('count', e.target.value)} required placeholder="0"
                className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" />
            </div>
          </div>
          <div>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="submit" disabled={saving}
              className="flex-1 py-2 bg-yellow-400 hover:bg-yellow-500 text-gray-900 rounded-lg text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-1">
...
export default function PoultryEggsPage() {
  ...
  async function load() {
    ...
    const r = await fetch(`${EGGS_API}?from=${from}&to=${to}`, { headers: { Authorization: `Bearer ${token()}` } });
    const j = await r.json();
    setEntries(j.data ?? []);
    setLoading(false);
  }
  ...
  async function del(id) {
    ...
    await fetch(`${EGGS_API}/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token()}` } });
    ...
    <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setModal('add')}
          className="flex items-center gap-1.5 px-4 py-2 bg-yellow-400 hover:bg-yellow-500 text-gray-900 rounded-lg text-sm font-medium">
          <Plus size={15} /> Log Eggs
        </button>
      </div>
      ...
      <div className="flex items-center justify-between bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl px-4 py-3">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-yellow-600 dark:text-yellow-400">{total} eggs total</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400 dark:text-gray-600">
          <Egg size={40} className="mx-auto mb-3 opacity-30" />
          <p className="text-sm">No entries for {monthLabel}.</p>
        </div>
      ) : (
          ...
          {entries.map(e => (
            <div key={e.id} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl px-4 py-3 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 dark:text-white">{e.count} eggs</p>
