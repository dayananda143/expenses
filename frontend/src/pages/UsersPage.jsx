...
import { Plus, Trash2, X, ShieldCheck, ShieldOff, UserCheck, UserX, CreditCard, Pencil } from 'lucide-react';
...
function NewUserModal({ onClose }) {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors, isSubmitting } } = useForm({
    defaultValues: { workspaces: ['india', 'us'], accounts_access: false },
  });
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4">
          ...
          <div>
            ...
            <input type="password" {...register('password', { required: 'Required', minLength: { value: 4, message: 'Min 4 chars' } })} className={inputCls} />
...
function EditUserModal({ user, onClose }) {
  ...
  async function onSubmit(data) {
    try {
      const payload = { username: data.username, workspaces: data.workspaces, accounts_access: data.accounts_access, is_admin: data.is_admin };
