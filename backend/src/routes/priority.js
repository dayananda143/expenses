...
router.get('/', (req, res, next) => {
  try {
    const rows = db.prepare(
      'SELECT * FROM priority_items WHERE user_id = ? AND workspace = ? ORDER BY sort_order ASC, created_at ASC'
...
router.post('/', (req, res, next) => {
  try {
    ...
    const maxOrder = db.prepare('SELECT COALESCE(MAX(sort_order),0) AS m FROM priority_items WHERE user_id = ? AND workspace = ?').get(req.user.id, req.workspace).m;
    const result = db.prepare(
      'INSERT INTO priority_items (user_id, workspace, name, budget, saved, notes, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).run(req.user.id, req.workspace, name.trim(), parseFloat(budget), parseFloat(saved ?? 0), notes?.trim() || null, maxOrder + 1);
...
router.put('/:id', (req, res, next) => {
  try {
    const item = db.prepare('SELECT * FROM priority_items WHERE id = ? AND user_id = ? AND workspace = ?').get(req.params.id, req.user.id, req.workspace);
...
router.delete('/:id', (req, res, next) => {
  try {
    db.prepare('DELETE FROM priority_items WHERE id = ? AND user_id = ? AND workspace = ?').run(req.params.id, req.user.id, req.workspace);
    res.status(204).send();
  } catch (err) { next(err); }
});
...
router.patch('/reorder', (req, res, next) => {
  try {
    ...
    const update = db.prepare('UPDATE priority_items SET sort_order = ? WHERE id = ? AND user_id = ? AND workspace = ?');
