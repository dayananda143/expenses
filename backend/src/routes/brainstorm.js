...
router.get('/', (req, res, next) => {
  try {
    const rows = db.prepare(
      'SELECT * FROM brainstorm_items WHERE workspace = ? ORDER BY sort_order ASC, created_at ASC'
    ).all(req.workspace);
    res.json({ data: rows });
  } catch (err) { next(err); }
});
...
router.post('/', (req, res, next) => {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO brainstorm_items (user_id, workspace, name, total_amount, paid_amount, notes, sort_order) VALUES (?, ?, ?, ?, 0, ?, ?)'
    ).run(req.user.id, req.workspace, name.trim(), Number(total_amount), notes?.trim() || null, maxOrder + 1);
...
router.put('/:id', (req, res, next) => {
  try {
    ...
    if (!total_amount || isNaN(Number(total_amount)) || Number(total_amount) <= 0)
      return res.status(400).json({ error: 'total_amount must be a positive number' });
    db.prepare('UPDATE brainstorm_items SET name=?, total_amount=?, notes=? WHERE id=?')
      .run(name.trim(), Number(total_amount), notes?.trim() || null, item.id);
...
router.post('/:id/records', (req, res, next) => {
  try {
    ...
    const result = db.prepare(
      'INSERT INTO brainstorm_records (item_id, name, amount) VALUES (?, ?, ?)'
    ).run(item.id, name.trim(), Number(amount));
