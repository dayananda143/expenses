...
const adminUserWhere = 'p.user_id IN (SELECT id FROM users WHERE is_admin = 1)';
const adminAccountWhere = 'user_id IN (SELECT id FROM users WHERE is_admin = 1)';
...
router.post('/', (req, res, next) => {
  ...
  try {
    ...
    const account = db.prepare(`SELECT * FROM accounts WHERE id = ? AND ${adminAccountWhere} AND workspace = ?`)
      .get(account_id, req.workspace);
    ...
    ).run(account_id, adminId, req.workspace, paid, date, notes ?? null);
...
router.delete('/:id', (req, res, next) => {
  if (!req.user.is_admin) return res.status(403).json({ error: 'Admin only' });
  try {
    const payment = db.prepare(`SELECT p.* FROM account_payments p WHERE p.id = ? AND ${adminUserWhere} AND p.workspace = ?`)
    ...
    const latest = db.prepare('SELECT MAX(date) AS d FROM account_payments WHERE account_id = ?').get(payment.account_id);
